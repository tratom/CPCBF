/*
 * CPCBF — Arduino MKR WAN 1300 Agent (LoRa)
 * JSON command-response loop over Serial (USB), identical protocol to the
 * MKR WiFi 1010 agent. Log lines prefixed with '# ' go to stderr via the
 * serial relay; JSON lines (starting with '{') are the command channel.
 */
#include <Arduino.h>
...
#ifdef CPCBF_PROTOCOL_LORA
extern "C" protocol_adapter_t lora_adapter;
#endif
...
static void send_ok(const char *message) { send_response("ok", message); }
static void send_error(const char *message) { send_response("error", message); }
...
static void handle_configure(JsonObject params)
{
    ...
    /* LoRa is always P2P — honour whatever the controller passes but
     * default to P2P. */
    g_adapter_cfg.topology = TOPO_P2P;

#ifdef CPCBF_PROTOCOL_LORA
    g_active_adapter = &lora_adapter;
#endif
    ...
    g_test_cfg.payload_size = params["payload_size"] | (uint16_t)32;
    ...
    /* Flood mode: aggregate-only to avoid filling SRAM */
...
#ifdef CPCBF_PROTOCOL_LORA
    data["lora_active"] = g_adapter_ready ? true : false;
#endif
    /* Always false — MKR WAN 1300 has no WiFi/BT on-board. Reported for
     * symmetry with the isolation preflight shape. */
...
{
    if (!g_adapter_ready || !g_active_adapter) {
        send_error("adapter not ready — run LORA_SETUP first");
        return;
    }
...
static void handle_start()
{
    ...
    if (!g_adapter_ready) {
#ifdef CPCBF_PROTOCOL_LORA
        if (!g_active_adapter) g_active_adapter = &lora_adapter;
#endif
        int rc = g_active_adapter->init(g_active_adapter, &g_adapter_cfg);
        if (rc != ADAPTER_OK) {
            send_error("adapter init failed");
...
void loop()
{
    ...
    if (strcmp(command, "CONFIGURE") == 0) {
        ...
        handle_radio_status();
    } else if (strcmp(command, "LORA_SETUP") == 0) {
        handle_link_setup();
